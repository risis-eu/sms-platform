        ...
        if (resource === 'dataset.resourcesByType') {
            ...
            getDynamicEndpointParameters(user, datasetURI, (endpointParameters)=>{
                ...
                configurator.prepareDatasetConfig(user, 1, datasetURI, (rconfig)=> {
                    ...
                    let page = params.page ? params.page : 1;
                    let offset = (page - 1) * maxOnPage;
                    query = queryObject.getResourcePropForAnnotation(endpointParameters, graphName,  rconfig, resourceType, propertyURI, maxOnPage, offset, params.inNewDataset, params.boundarySource, params.longPropertyURI, params.latPropertyURI, params.countryPropertyURI);
                    //console.log(query);
                    //build http uri
                    //send request
                    rp.get({uri: getHTTPGetURL(getHTTPQuery('read', query, endpointParameters, outputFormat)), headers: headers}).then(function(res){

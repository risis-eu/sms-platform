    ...
    if(payload.mode === 'init'){
        ...
        async.parallel([
            ...
            (cback) => {
                ...
                context.service.read('facet.facetsSecondLevel', payload, {timeout: 80 * 1000}, function (err, res) {
        ...
        async.parallel([
            (cback) => {
                ...
                context.service.read('facet.facetsMasterCount', payload, {timeout: 80 * 1000}, function (err, res) {
                    //end = new Date().getTime();
                    //timeElapsed = end - start;
                    if (err) {
            ...
            (cback) => {
                //items of facets
                context.service.read('facet.facetsMaster', payload, {timeout: 80 * 1000}, function (err, res) {
                    //end = new Date().getTime();
                    //timeElapsed = end - start;
                    if (err) {
                        context.dispatch('LOAD_FACETS_FAILURE', err);
        ...
        async.parallel([
            (cback) => {
                //total number of items listed in facet
                context.service.read('facet.facetsSideEffectCount', payload, {timeout: 80 * 1000}, function (err, res) {
                    //end = new Date().getTime();
                    //timeElapsed = end - start;
                    if (err) {
                        context.dispatch('LOAD_FACETS_FAILURE', err);
            ...
            (cback) => {
                //items of facets
                context.service.read('facet.facetsSideEffect', payload, {timeout: 80 * 1000}, function (err, res) {
                    //end = new Date().getTime();
                    //timeElapsed = end - start;
                    if (err) {
                        context.dispatch('LOAD_FACETS_FAILURE', err);

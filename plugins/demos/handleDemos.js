...
module.exports = function handleDemos(server) {
    ...
    server.get('/demos/geo', function(req, res) {
        res.json({
            'message': 'Hello! I am a demo Geo app! Try this example: /demos/geo/NUTS/NL326'
        });
    });
    ...
    server.get('/demos/geo/PointToNUTS/:long?/:lat?', function(req, res) {
        ...
        rp.get({
            ...
            regions.forEach(function(item) {
                regionLinks[parseInt(item.level)] = {
                    id: item.code,
                    title: item.name,
                    uri: item.uri
                };
        ...
        rp.get({
            ...
            codes.forEach(function(item) {
                ...
                asyncTasks.push(function(callback) {
                    rp.get({
                        uri: 'http://' + req.headers.host + smsAPI + '/geo.NUTStoPolygon;smsKey=' + demoSMSKey + ';code=' + item.code
                    }).then(function(body2) {
        ...
        rp.get({
            ...
            regions.forEach(function(item) {
                if (!regionLinks[parseInt(item.level)]) {
                    ...
                    regionLinks[parseInt(item.level)].push({
                        id: item.id,
                        title: item.title
                    });
                }
    ...
    server.get('/demos/geo/PointToGADM28AdminMap/:long?/:lat?/:country?/:width?/:height?/:sep?', function(req, res) {
        ...
        rp.get({
            ...
            codes.forEach(function(item) {
                ...
                asyncTasks.push(function(callback) {
                    rp.get({
                        uri: 'http://' + req.headers.host + smsAPI + '/geo.GADM28AdminToPolygon;smsKey=' + demoSMSKey + ';id=' + item.id
                    }).then(function(body2) {
                        var parsed2 = JSON.parse(body2);
                        var input = parsed2.resources[0].polygon;
                        polygons.push(input);
    ...
    server.get('/demos/geo/addressToGADM28Admin', function(req, res) {
        res.render('addressToGADM28Admin', {
            input: '',
    ...
    server.post('/demos/geo/addressToGADM28Admin', function(req, res) {
        ...
        rp.get({
            ...
            if (parsed.resources.results.length) {
                ...
                res.render('addressToGADM28Admin', {
                    input: req.body.addr,
                    address: encodeURIComponent(req.body.addr),
                    point: {
                        long: longitude,
                        lat: latitude,
                        country: country
        ...
        rp.get({
            ...
            rp.get({
                ...
                regionLinks.forEach(function(item, i) {
                    ...
                    out = out + '<div class="ui item" title="' + (lmetadata && lmetadata.resource && lmetadata.resource['level' + i] ? lmetadata.resource['level' + i].replace(/"/g, '&quot;') : 'no metadata available!') + '"><span class="ui mini teal circular label">' + i + '</span>' + dv + ' ' + itemDIV.join(' | ') + '</div>';
                    dv = dv + '-';
                });
    ...
    server.get('/demos/geo/addressToFlickrAdmin', function(req, res) {
        res.render('addressToFlickrAdmin', {
            input: '',
            appShortTitle: appShortTitle,
            appFullTitle: appFullTitle
        });
    ...
    server.get('/demos/geo/AdminCountryLevels/:country/:level/:source/:width?/:height?/:offset?/:limit?', function(req, res) {
        ...
        rp.get({
            ...
            codes.forEach(function(item) {
                ...
                asyncTasks.push(function(callback) {
                    rp.get({
                        uri: 'http://' + req.headers.host + smsAPI + '/geo.' + sourceSTR + ';smsKey=' + demoSMSKey + ';id=' + item.id
                    }).then(function(body2) {
                        rp.get({
                            uri: 'http://' + req.headers.host + smsAPI + '/geo.' + sourceSTR + 'ToPolygon;smsKey=' + demoSMSKey + ';id=' + item.id
                        }).then(function(body3) {
                            var parsed2 = JSON.parse(body2);
            ...
            async.parallelLimit(asyncTasks, 20, function() {
                ...
                polygons.forEach(function(input, i) {
                    ...
                    features.push({
                        ...
                        'geometry': {
                            'type': parsedC.shapeType,
                            coordinates: [parsedC.coordinatesArr]
                        }
                    });
                });
                var focusPoint;
    ...
    server.get('/demos/geo/NL_Universities/:width?/:height?', function(req, res) {
        ...
        nl_unis_osm_polygons.forEach((uni) => {
            if (!flagID[uni.processed.id]) {
                ...
                features.push({
                    ...
                    'id': uni.processed.id,
                    'properties': {
                        'name': uni.processed.resources[0].name,
                        source: 'osm'
                    },
                    'geometry': {
                        'type': parsedC.shapeType,
        ...
        nl_universities.forEach((uni) => {
            ...
            features.push({
                ...
                'geometry': {
                    'type': 'Point',
                    coordinates: [uni.processed.geometry.location.lng, uni.processed.geometry.location.lat]
                }
            });
        });
        var focusPoint;
    ...
    server.get('/demos/geo/DetectOECDFUAs/:country/:list', function(req, res) {
        ...
        list.forEach(function(item) {
            ...
            asyncTasks.push(function(callback) {
                rp.get({
                    uri: 'http://' + req.headers.host + smsAPI + '/geo.BoundaryToOECDFUA' + ';smsKey=' + demoSMSKey + ';name=' + item + ';country=' + country
                }).then(function(body2) {
                    var parsed2 = JSON.parse(body2);
        ...
        async.parallelLimit(asyncTasks, 2, function() {
            ...
            output.forEach(function(input, i) {
                if (input.resources.length && logitems.indexOf(input.resources[0].funactionalUrbanArea.code) == -1) {
    ...
    server.post('/demos/geo/exportPointsToGeoJSON/:bindProps?', function(req, res) {
        ...
        points.forEach(function(point, i) {
            if (flags[point.id]) {
                ...
                flags[point.id] = {
                    'frequency': 1,
                    'relations': (point.relation ? [point.relation] : [])
                };
                features.push({
                    'type': 'Feature',
                    'id': point.id,
    ...
    server.post('/demos/geo/exportToGeoJSON/:bindProps?', function(req, res) {
        ...
        boundaries.forEach(function(boundary, i) {
            if (flags[boundary.id]) {
                ...
                flags[boundary.id] = {
                    'frequency': 1,
                    'relations': (boundary.relation ? [boundary.relation] : [])
                };
                // We add a function containing it to an array of "tasks"
                asyncTasks.push(function(callback) {
                    rp.get({
                        uri: apiURI + boundary.id
                    }).then(function(body) {
                        var parsed = JSON.parse(body);
